...
theorem fnLb_add {f g : ℝ → ℝ} {a b : ℝ}  (hfa : FnLb f a) (hgb : FnLb g b) : FnLb (fun x ↦ f x + g x) (a + b) := by
  intro x
  dsimp
  apply add_le_add
  · apply hfa
...
example (divab : a ∣ b) (divbc : b ∣ c) : a ∣ c := by
  ...
  rcases divbc with ⟨e, ceq, rfl⟩
  use d * e; ring

example (divab : a ∣ b) (divac : a ∣ c) : a ∣ b + c := by
  rcases divab with ⟨d, h1, rfl⟩
  rcases divac with ⟨e, h2, rfl⟩
  use d+e
  ring
...
example (surjg : Surjective g) (surjf : Surjective f) : Surjective fun x ↦ g (f x) := by
  intro x
  rcases surjg x with ⟨y, h1⟩
  rcases surjf y with ⟨z, h2⟩
  use z
  dsimp
  rw [h2, h1]
